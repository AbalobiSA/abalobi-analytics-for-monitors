(function(){
    ...
    var mcsbptController = function MonthlyCatchSpeciesByPermitTypeController(MonitorResource, SpeciesUtil, StringUtil) {
        ...
        function requestData(calcMethod){
            ctrl.loading = true;
            MonitorResource.query({queryType: "total_species_weight_by_month_by_permit_type", parameter: calcMethod})
        ...
        function handleCatchResponse(data) {
            ...
            permitTypes = d3.set(data, x => x.permit_type)
        ...
        function collectTotal(acc, entry){
            acc.set(entry.permit_type, acc.get(entry.permit_type)+entry[selectedCalculationMethod]);
            return acc;
        }
    }

    angular.module('monthlyCatchSpeciesByPermitTypeModule')
        .component('monthlyCatchSpeciesByPermitData', {
